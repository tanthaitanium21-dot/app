// systems/electrical/main.js
// เวอร์ชัน V3.0 (Integrated): รวม Logic เดิม V2.5 + เชื่อมต่อ Core System

// 1. [NEW] Import ข้อมูลและเครื่องมือจาก Core System
import { provinces, provinceZones } from '../../js/data/shared_config.js';
import { formatCurrency, saveProjectToFile, loadProjectFromFile, exportToPDF, exportToImage } from '../../js/utils.js';
import { authSystem } from '../../js/auth.js';

// 2. Import ข้อมูลเฉพาะของระบบไฟฟ้า
import { mainCableSpecs } from './data/electrical_data.js';

// 3. Import Modules (Logic การคำนวณเดิม)
import { calculateProjectCost, setPrice, getPriceList } from './modules/calculator.js';
import { renderCircuitInputs, renderDedicatedCircuitInputs, renderDynamicInputs } from './modules/ui_renderer.js'; // ตัด formatCurrency ออกเพราะใช้จาก utils
...
function initApp() {
    console.log("Electrical System V3.0 Starting...");

    // [NEW] ตรวจสอบสิทธิ์ก่อนเริ่ม (ถ้ายังไม่ Login จะแจ้งเตือน)
    authSystem.checkAccess();
...
function handleProvinceChange() {
    ...
    if(bkkZoneContainer) {
        ...
        // หมายเหตุ: Logic เดิมคือ hidden ถ้าไม่ใช่ กทม. แต่ถ้าอยากให้โชว์เฉพาะ กทม. ต้องแก้เป็น:
        // bkkZoneContainer.classList.toggle('hidden', province !== 'กรุงเทพมหานคร');
        // (โค้ดเดิมของคุณใช้ !== 'กรุงเทพมหานคร' คือซ่อนถ้าไม่ใช่ กทม. ซึ่งถูกต้องแล้ว)
         bkkZoneContainer.classList.toggle('hidden', province !== 'กรุงเทพมหานคร');
    }

    if(authEl) {
        // [UPDATED] ใช้ provinceZones จาก shared_config
        const isMEA = provinceZones.MEA && provinceZones.MEA.includes(province);
...
// --- Logic หัวใจหลัก: การดึงค่าจาก DOM เพื่อคำนวณ ---
...
// [UPDATED] Setup Export Buttons using Central Utils
function setupExportButtons() {
    document.getElementById('save-pdf-btn')?.addEventListener('click', () => {
        const btn = document.getElementById('save-pdf-btn');
        btn.innerText = "Processing..."; btn.disabled = true;
        exportToPDF('output-section', 'electrical_quotation.pdf').then(() => {
             btn.innerText = "PDF"; btn.disabled = false;
        });
    });
    
    document.getElementById('save-image-btn')?.addEventListener('click', () => {
        exportToImage('output-section', 'electrical_quotation.png');
    });
}

// [NEW] Setup Save/Load Buttons
function setupSaveLoadButtons() {
    document.getElementById('btn-save-project')?.addEventListener('click', () => {
        ...
        saveProjectToFile(inputs, 'electrical_project.json');
    });

    document.getElementById('btn-load-project')?.addEventListener('change', (e) => {
        if(e.target.files[0]) {
            loadProjectFromFile(e.target.files[0]).then(data => {
