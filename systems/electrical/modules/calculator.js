// systems/electrical/modules/calculator.js

// [จุดเชื่อมสำคัญ] เปลี่ยน Path ไปดึงตัวคูณจาก Core Shared Data
// ต้องถอยกลับ 3 ชั้น: modules/ -> electrical/ -> systems/ -> Root -> js/data/
import { bkkZoneMultipliers, provinceMultipliers } from '../../../js/data/shared_config.js';

import { electricalInstallationData, initialPriceList } from '../data/electrical_data.js';

// ... (Logic การคำนวณด้านล่างเหมือนเดิม 100%) ...
// ... ระบบจะใช้ตัวแปร bkkZoneMultipliers และ provinceMultipliers ใหม่ที่ Import มาอัตโนมัติ ...
...
export function calculateProjectCost(taskQuantities, settings, manualPO=[], manualBOQ=[]) {
            ...
            if (tid === '17.5' && cableSpecConfig) {
                ...
                if(selectedSize) {
                    ...
                    if(m.material_id === targetMatId) {
                        ...
                        const gndId = 'M-WIRE-GND-10SQMM'; // สมมติเป็นเบอร์นี้สำหรับเมน
    ...
    // ใช้ตัวแปรที่ Import มาจาก Shared Config
