            ...
            <div class="circuit-container bg-gray-50 p-4 rounded-lg border border-gray-200 mb-3 shadow-sm">
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div id="${prefix}_${i}_breaker_info" class="bg-green-50 border border-green-200 rounded-md p-3 text-sm flex flex-col gap-1 shadow-sm">
                        ...
                        <div class="text-slate-600 pl-6 text-xs" id="${prefix}_${i}_info_detail">กำลังโหลดข้อมูลราคา...</div>
                    </div>
                    <div class="grid grid-cols-2 gap-3 bg-white p-3 rounded border border-slate-200 mt-2">
                        <div><label class="text-[10px] text-slate-500">ระยะ ตู้ไฟ→เบรกเกอร์ (ม.)</label><input type="number" id="${prefix}_${i}_panel_to_breaker_dist" class="form-input w-full text-sm" placeholder="0"></div>
                        <div><label class="text-[10px] text-slate-500">ระยะ เบรกเกอร์→เครื่อง (ม.)</label><input type="number" id="${prefix}_${i}_breaker_to_unit_dist" class="form-input w-full text-sm" placeholder="0"></div>
                    </div>
                    <div><label class="text-xs text-slate-600">ระยะสายดิน ตู้ไฟ→เครื่อง (ม.)</label><input type="number" id="${prefix}_${i}_panel_to_unit_dist_ground" class="form-input mt-1 w-full text-sm" placeholder="0"></div>
                </div>
            </div>`;
    }
    container.innerHTML = html;
